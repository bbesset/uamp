...
fun MediaMetadataCompat.toMediaSource(dataSourceFactory: DataSource.Factory): MediaSource{
    val factory = if(mediaUri.toString().endsWith(".mp3"))
        ProgressiveMediaSource.Factory(dataSourceFactory).setTag(fullDescription)
    else
        HlsMediaSource.Factory(dataSourceFactory).setTag(fullDescription)
